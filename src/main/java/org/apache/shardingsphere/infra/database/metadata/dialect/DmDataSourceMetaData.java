...
public final class DmDataSourceMetaData implements DataSourceMetaData {
    ...
    private final Pattern thinUrlPattern = Pattern.compile("jdbc:dm:(//)?([\\w\\-\\.]+):?([0-9]*)[:/]([\\w\\-]+)", Pattern.CASE_INSENSITIVE);

    private final Pattern connectDescriptorUrlPattern = Pattern.compile("jdbc:dm:[(\\w\\s=)]+HOST\\s*=\\s*(\\d+(\\."

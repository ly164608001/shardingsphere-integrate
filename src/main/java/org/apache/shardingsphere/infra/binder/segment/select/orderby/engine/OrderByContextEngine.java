...
public final class OrderByContextEngine {
    ...
    public OrderByContext createOrderBy(final SelectStatement selectStatement, final GroupByContext groupByContext) {
        if( selectStatement.getWhere().isPresent() &&
                selectStatement.getWhere().get().getExpr() instanceof BinaryOperationExpression &&
                ((BinaryOperationExpression) selectStatement.getWhere().get().getExpr()).getText().contains("ROW_ID")) {
            return createOrderByContextForPage(selectStatement);
        }
    ...
    private OrderByContext createOrderByContextForPage(final SelectStatement selectStatement) {
        Optional<OrderBySegment> orderBy = ((SubqueryTableSegment) ((SubqueryTableSegment) selectStatement.getFrom()).getSubquery().getSelect().getFrom()).getSubquery().getSelect().getOrderBy();
        if(orderBy.isPresent()) {
            List<OrderByItem> orderByItems = new LinkedList<>();
            for (OrderByItemSegment each : orderBy.get().getOrderByItems()) {
                OrderByItem orderByItem = new OrderByItem(each);
                if (each instanceof IndexOrderByItemSegment) {
                    orderByItem.setIndex(((IndexOrderByItemSegment) each).getColumnIndex());
                }
        ...
        return new OrderByContext(Collections.emptyList(), false);
    }
}

...
public final class RowNumberPaginationContextEngine {
    ...
    static {
        ...
        ROW_NUMBER_IDENTIFIERS.add("ROW_ID");
    }
    ...
    public PaginationContext createPaginationContext(final Collection<ExpressionSegment> expressions, final ProjectionsContext projectionsContext, final List<Object> parameters) {
        Optional<String> rowNumberAlias = isRowNumberAlias(projectionsContext,expressions);
    ...
    private Optional<String> isRowNumberAlias(final ProjectionsContext projectionsContext,final Collection<ExpressionSegment> where) {
        ...
        for (String each : ROW_NUMBER_IDENTIFIERS) {
            if(Lists.newArrayList(where).get(0) instanceof BinaryOperationExpression
                    && ((BinaryOperationExpression) Lists.newArrayList(where).get(0)).getText().contains(each)) {
                return Optional.of("TMP_PAGE");
            }
        }

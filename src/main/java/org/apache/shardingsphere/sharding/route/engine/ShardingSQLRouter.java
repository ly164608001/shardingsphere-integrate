...
public final class ShardingSQLRouter implements SQLRouter<ShardingRule> {
    ...
    private boolean isNeedMergeShardingValues(final SQLStatementContext<?> sqlStatementContext, final ShardingRule rule) {
        ...
        return (selectContainsSubquery || insertSelectContainsSubquery) && !rule.getShardingLogicTableNames(sqlStatementContext.getTablesContext().getTableNames()).isEmpty()
                && isShardingTable(sqlStatementContext, rule);
    }

    //edit by ly Must have sharding column with subquery #4722 pending
    private boolean isShardingTable(final SQLStatementContext<?> sqlStatementContext, final ShardingRule rule) {
        TableRule tableRule = rule.getTableRule(sqlStatementContext.getTablesContext().getTableNames().iterator().next());
        Field actualTablesField = ReflectionUtils.findField(TableRule.class, "actualTables");
        actualTablesField.setAccessible(true);
        try {
            Set<String> actualTables = (Set<String>)actualTablesField.get(tableRule);
    ...
    private boolean isSameShardingCondition(final ShardingRule shardingRule, final ShardingCondition shardingCondition1, final ShardingCondition shardingCondition2) {
        ...
        for (int i = 0; i < shardingCondition1.getValues().size(); i++) {
            ...
            //edit by ly 2020-03-25 fix subquery with rangeRouteValue
            if(shardingConditionValue1 instanceof RangeShardingConditionValue && isSameShardingConditionValue((RangeShardingConditionValue) shardingConditionValue1, (RangeShardingConditionValue) shardingConditionValue1)){
                return true;
            }else if(!isSameShardingConditionValue(shardingRule, (ListShardingConditionValue) shardingConditionValue1, (ListShardingConditionValue) shardingConditionValue2)) {
                return false;
            }
        }
        return true;
    ...
    private boolean isSameShardingConditionValue(final RangeShardingConditionValue routeValue1, final RangeShardingConditionValue routeValue2) {
        return routeValue1.getTableName().equals(routeValue2.getTableName())
                && routeValue1.getColumnName().equals(routeValue2.getColumnName()) && routeValue1.getValueRange().isConnected(routeValue2.getValueRange());
    }

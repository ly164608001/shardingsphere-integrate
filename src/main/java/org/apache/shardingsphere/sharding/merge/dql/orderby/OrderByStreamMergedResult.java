...
public class OrderByStreamMergedResult extends StreamMergedResult {
    ...
    public OrderByStreamMergedResult(final List<QueryResult> queryResults,final SelectStatementContext selectStatementContext, final PhysicalSchemaMetaData schemaMetaData, boolean isCountSql) throws SQLException {
        orderByItems = selectStatementContext.getOrderByContext().getItems();
    ...
    private void orderResultSetsToQueue(final List<QueryResult> queryResults, final SelectStatementContext selectStatementContext, final PhysicalSchemaMetaData schemaMetaData) throws SQLException {
        ...
        if(isCountSql) {
            Long count = 0L;
            for (OrderByValue orderByValue : orderByValuesQueue) {
                count += (Long)orderByValue.getQueryResult().getValue(1,Long.class);
            }
            MemoryQueryResult queryResult = (MemoryQueryResult)orderByValuesQueue.peek().getQueryResult();
            queryResult.getCurrentRow().clear();
            queryResult.getCurrentRow().add(count);
        }
